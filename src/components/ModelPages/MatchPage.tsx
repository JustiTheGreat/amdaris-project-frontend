...
import { CompetitionStatus, MatchGetDTO, MatchStatus, PointDisplayDTO } from "../../utils/Types";
...
export const MatchPage: FC = () => {
  ...
  const cancelRequest = () =>
    requests.cancelMatchRequest({ id }, (data: any) => {
      setMatch(data);
      setFirstTabSwitch(!firstTabSwitch);
    });
  ...
  const getTabInfoList = useCallback((): TabInfo[] => {
        ...
        >
          ...
          >
            <Button
              variant="outlined"
              size="large"
              sx={{ fontSize: "32px", fontWeight: "bold", display: "flex", gap: (theme) => theme.spacing(2) }}
              onClick={() => navigate(`/${competitorPath}/${match.competitorOne.id}`)}
            >
              <ProfilePictureContainer src={match.competitorOne.profilePicture} />
            ...
            >
              <ProfilePictureContainer src={match.competitorTwo.profilePicture} />
          ...
          {(match.status === MatchStatus.FINISHED ||
            ...
            >
              ...
              {match.winner ? (
                ...
                >
                  <ProfilePictureContainer src={match.winner.profilePicture} />
                  {match.winner.name}
                </Button>
              ) : (
                "-"
          ...
          <Box sx={{ height: (theme) => theme.spacing(5) }} />
          ...
          <Box sx={{ display: "flex", gap: (theme) => theme.spacing(2) }}>
            ...
            <Box>
              ...
              <Typography>
                Score: {`${match.competitorOnePoints ?? ""} - ${match.competitorTwoPoints ?? ""}`}
              </Typography>
              <Typography>
                Competition:
                <Button
    ...
    if (isAdmin)
      tabInfoList.push({
        ...
        content: match.competition.status === CompetitionStatus.STARTED &&
